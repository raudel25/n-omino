...
public abstract class Player<T>
{
    ...
    protected List<Move<T>> GetValidMoves(Hand<T> myHand, GameStatus<T> status, InfoRules<T> rules)
    {
    ...
    public abstract Move<T> Play(GameStatus<T> status, InfoRules<T> rules);
}
...
public class EvaluatePlayer<T> : Player<T>
{
    ...
    public override Move<T> Play(GameStatus<T> status, InfoRules<T> rules)
    {
        var myHand = status.Players[status.FindPLayerById(Id)].Hand;
        var validMoves = GetValidMoves(myHand, status, rules);
