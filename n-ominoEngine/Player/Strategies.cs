...
{
    public IEnumerable<Move<T>> Play(IEnumerable<Move<T>> possibleMoves, GameStatus<T> status, InfoRules<T> rules,
        int id);
}
...
public class RandomPlayer<T> : IStrategy<T>
{
    ...
    IEnumerable<Move<T>> IStrategy<T>.Play(IEnumerable<Move<T>> possibleMoves, GameStatus<T> status, InfoRules<T> rules,
        int id)
    {
...
public class TeamPlayer<T> : IStrategy<T>
{
    ...
    public IEnumerable<Move<T>> GetFavTeamMoves(IEnumerable<Move<T>> possibleMoves, GameStatus<T> status,
        InfoRules<T> rules, int id)
    {
        ...
        foreach (var item in possibleMoves.Where(x =>
                     x.Token!.Contains(value) && !Mata(x.Node!, value) && !MataTeam(x.Node!, id, status)))
            yield return item;
    }
    ...
    public bool Mata(INode<T> node, T value)
    {
        ...
        for (var i = 0; i < node.Connections.Length; i++)
        {
            ...
            if (Equals(node.Connections[i]!.ValueToken[i], value)) return true;
        }
    ...
    public IEnumerable<(T value, int cant)> GetTeamPuestas(GameStatus<T> status, int team)
    {
        ...
        {
            var sum = 0;
            foreach (var player in status.Teams[team]) sum += player.History.HowManyPuso(value);
...
public class EnemyPlayer<T> : IStrategy<T>
{
    ...
    public IEnumerable<Move<T>> GetKillEnemyMoves(IEnumerable<Move<T>> possibleMoves, GameStatus<T> status,
        InfoRules<T> rules, int id)
    {
        foreach (var (value, cant) in GetEnemyTeamsMatadas(status, status.FindTeamPlayer(id))
    ...
    public IEnumerable<(T value, int cant)> GetEnemyTeamsMatadas(GameStatus<T> status, int team)
    {
        ...
        {
            var sum = 0;
            foreach (var player in Enemies(status, team)) sum += player.History.HowManyMato(value);
            yield return (value, sum)!;
        }
    }
    ...
    {
        for (var i = 0; i < status.Teams.Count; i++)
        {
            if (i == team) continue;
            foreach (var player in status.Teams[i]) yield return player;
        }
    }
}
...
public class SinglePlayer<T> : IStrategy<T>
{
        ...
        int id)
    {
        ...
        var moves = GetMovesWithValue(values.First().value, possibleMoves);
    ...
    public IEnumerable<Move<T>> GetMovesWithValue(T value, IEnumerable<Move<T>> possiblemoves)
    {
        return possiblemoves.Where(move => move.Token!.Contains(value));
    }
